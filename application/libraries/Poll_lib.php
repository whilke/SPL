...
class Poll_lib {
	...
	public function vote($poll_id, $option_id, $user_id)
	{
                ...
                //only logged in people can vote
                if ($user_id < 1)
                {
                    $this->set_error('error_not_logged_in');
                    return FALSE;
                }
		...
		if ($this->is_closed($poll_id))
		{
			...
			return FALSE;
                ...
                //only players who have played in a valid SPL match may vote.
                if (!$this->is_valid_player($user_id))
                {
                    $this->set_error('error_not_player');
                    return FALSE;
                }
		
		if ($this->allow_multiple_votes === TRUE)
		{
			if ( ! $this->has_previously_voted_within($this->interval_between_votes, $poll_id, $user_id))
			{
				$this->add_vote($option_id, $user_id);
				return TRUE;
			}
			else
			{
		...
		{
			if ( ! $this->has_previously_voted($poll_id, $user_id))
			{
				$this->add_vote($option_id, $user_id);
				return TRUE;
			}
